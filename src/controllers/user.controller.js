...
const updateuserAvatar = asyncHandler( async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    .status(200)
    .json(
        new ApiResponce(201,user, "Avatar updated successfully")
    )
})
...
const updateUserCoverImage = asyncHandler( async (req, res) => {
    ...
    if(!coverImage.url){
        throw new ApiError(400, "Error while uploading on imageCover")
    } 
    ...
    .status(200)
    .json(
        new ApiResponce(201,user, "CoverImage updated successfully")
    )
})
